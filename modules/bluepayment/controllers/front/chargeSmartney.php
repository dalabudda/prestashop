...
class BluePaymentChargeSmartneyModuleFrontController extends ModuleFrontController
{
    ...
    private function validateTransaction($transaction, $orderId)
    {
        ...
        if (isset($transaction->payment_status) && $transaction->payment_status == 'SUCCESS') {
            $array = [
                'status'      => 'SUCCESS',
                'message'     => $this->module->l('Payment has been successfully completed.', 'chargesmartney'),
        ...
        if (isset($transaction->payment_status) && $transaction->payment_status == 'PENDING') {
            $array = [
                'status'      => 'PENDING',
                'message'     => $this->module->l('We are waiting for payment confirmation', 'chargesmartney'),
                'transaction' => $transaction
            ];
        }
        if (isset($transaction->created_at) &&
            ...
            $array = [
                'status'  => 'FAILURE',
                'message' => $this->module->l('Transaction failed.', 'chargesmartney'),
            ];
        ...
        if (empty($array)) {
            $array = [
                'status'  => 'FAILURE',
                'message' => $this->module->l('The code has expired. Try again.', 'chargesmartney'),
            ];
        }
    ...
    private function validateRequest($response, $orderId)
    {
        ...
        if (!isset($response->confirmation)) {
            if ($response->status == 'PENDING') {
                $array = [
                    'status'  => 'PENDING',
                    'message' => $this->module->l('Transaction in progress.', 'chargesmartney'),
                ];
                ...
                $array = [
                    'status'  => 'FAILURE',
                    'message' => $this->module->l('An error occurred during the transaction.', 'chargesmartney'),
                ];
            }
            ...
            $array = [
                'status'  => 'FAILURE',
                'message' => $this->module->l('Your transaction has already been paid.', 'chargesmartney'),
            ];
        ...
        if (empty($array)) {
            $array = [
                'status'  => 'FAILURE',
                'message' => $this->module->l('Pay Smartney token has expired. Try again.', 'chargesmartney'),
            ];
        }
